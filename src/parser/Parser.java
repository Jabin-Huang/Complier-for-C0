...
import AST.Node;
import AST.expr.*;
import AST.expr.logical.And;
import AST.expr.logical.Not;
import AST.expr.logical.Or;
import AST.expr.logical.Rel;
import AST.expr.op.Access;
import AST.expr.op.Arith;
import AST.expr.op.Unary;
import AST.stmt.*;
import lexer.*;
...
public class Parser {
    ...
    private Stmt If() throws IOException {
        ...
        //stmt -> if (exprEntry) stmt else stmt
        match(Tag.ELSE);
        Stmt s2 = stmt();
        return new If_else(x, s1, s2);
    }

    //stmt -> while (exprEntry) stmt
    ...
    private Stmt assign(Word id_word) throws IOException {
        ...
        if (look.tag == '=') {
            move();
            stmt = new Assign(id, exprEntry());
        } else {
            //loc -> id offset
            Access x = offset(id);
            match('=');
            stmt = new SetElem(x, exprEntry());
        }
        match(';');
        return stmt;
    }
    ...
    /*
       exprEntry -> join || exprEntry
             | join
     */
    private Expr exprEntry() throws IOException {
        Expr x = join();
